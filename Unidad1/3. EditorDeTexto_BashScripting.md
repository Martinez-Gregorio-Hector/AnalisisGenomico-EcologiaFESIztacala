...
# El estilo C para el mando

Si has programado con el lenguaje C, probablemente te sorprenda cómo la shell bash usa el comando "for". En C, un bucle "for" normalmente define una variable, que se modifica automáticamente en cada iteración. Normalmente, los programadores usan esta variable como contador y lo incrementan o decrementan en uno en cada iteración. El comando "for" de bash también ofrece esta funcionalidad. Esta sección te muestra cómo usar un comando "for" de estilo C en un script de shell bash.
